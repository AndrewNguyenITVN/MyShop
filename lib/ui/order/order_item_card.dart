...
class _OrderItemCardState extends State<OrderItemCard> {
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: Column(
        children: <Widget>[
          OrderSummary(order: widget.order, expanded: _expanded, onExpandPressed: () {
            setState(() {
              _expanded = !_expanded;
            });
          }),
...
class OrderItemList extends StatelessWidget {
  const OrderItemList({required this.order, super.key});
  final OrderItem order;
  @override
  Widget build(BuildContext context) {
    return Container(
...
class OrderSummary extends StatelessWidget {
  const OrderSummary({super.key, required this.order, required this.expanded, this.onExpandPressed});
  final OrderItem order;
  final bool expanded;
